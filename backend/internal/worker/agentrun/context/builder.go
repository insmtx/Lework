...
func (b *ContextBuilder) Prepare(ctx context.Context, req *agentrundomain.RunRequest) (*agentrundomain.RunRequest, error) {
	...
	logs.InfoContextf(ctx, "Agent context prepare completed: run_id=%s trace_id=%s system_prompt_len=%d elapsed=%s",
		cloned.RunID, cloned.TraceID, len(systemPrompt), time.Since(startedAt))
	return cloned, nil
}
...
func CloneRequest(req *agentrundomain.RunRequest) *agentrundomain.RunRequest {
	...
	cloned.Conversation.Messages = append([]agentrundomain.InputMessage{}, req.Conversation.Messages...)
	cloned.Input.Messages = append([]agentrundomain.InputMessage{}, req.Input.Messages...)
	cloned.Input.Attachments = append([]agentrundomain.Attachment{}, req.Input.Attachments...)
