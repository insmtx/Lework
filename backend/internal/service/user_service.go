...
func (s *userService) GetUser(ctx context.Context, id uint, githubLogin string) (*contract.UserInfo, error) {
	caller, _ := auth.FromContext(ctx)
	if caller == nil || caller.Uin == 0 {
		return nil, errors.New("user not authenticated")
	}
	...
	if id > 0 {
		...
		return nil, errors.New("id or github_login is required")
	}
...
func (s *userService) UpdateUser(ctx context.Context, id uint, req *contract.UpdateUserRequest) (*contract.UserInfo, error) {
	caller, _ := auth.FromContext(ctx)
	if caller == nil || caller.Uin == 0 {
		return nil, errors.New("user not authenticated")
	}
	...
	if err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var err error
		user, err = db.GetUserByID(ctx, tx, id)
		if err != nil {
			return err
		}
		if user == nil {
...
func (s *userService) DeleteUser(ctx context.Context, id uint) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		return db.DeleteUser(ctx, tx, id)
	})
}
