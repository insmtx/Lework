...
func (r *InteractionRouter) RequestApproval(ctx context.Context, req *ApprovalRequest) (*ApprovalDecision, error) {
...
func (r *InteractionRouter) RequestAnswer(ctx context.Context, req *QuestionRequest) (*QuestionAnswer, error) {
...
func (r *InteractionRouter) ResolveQuestion(requestID string, answers [][]string) error {
	...
	pending.ResultCh <- &QuestionAnswer{
		RequestID: requestID,
		Answers:   answers,
	}
	return nil
...
// DefaultInteractionRouter 是全局交互路由器实例，供 HTTP handler、worker subscriber 和 runtime 共享。
// 统一管理审批和问题两种交互类型。
var DefaultInteractionRouter = NewInteractionRouter()
