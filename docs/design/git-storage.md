...
当前 Lework 的产物文件存在 worker 本地 workspace，通过 server 与 worker 共享挂载目录读取（参见 `architecture/workspace-artifact.md`）。该方式存在以下问题：
...
- **对外暴露 gitea 原生能力**：文件浏览、下载、预览直接复用 gitea API，Lework 不再自建文件树/产物接口。
...
| 项目记忆 | 纯 Gitea | Worker | `.lework/memory/` 目录，均为 Markdown 文件，无大文件场景 |
...
storage:
  ...
  s3:
    ...
    secret_key: <secret>
    use_ssl: false
    bucket: lework-artifacts
  gitea:
    endpoint: https://gitea.example.com
    admin_token: <token>
    default_owner: lework-system
    org_prefix: lework
```
...
```
          ...
          ▼                 ▼                 ▼
   ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
   │ Lework Server │  │ Gitea API    │  │ S3 Presigned │
   │ (鉴权+签发)  │  │ (文件浏览)   │  │ (大文件直连) │
   └──────┬───────┘  └──────┬───────┘  └──────┬───────┘
          │                 │                 │
          ▼                 ▼                 ▼
   ...
   │  + git 仓库  │  │ project 仓库 │  │ lework bucket │
   │  管理        │  │              │  │              │
   └──────────────┘  └──────────────┘  └──────────────┘
```
...
sequenceDiagram
    ...
    participant S as Lework Server
...
**仓库命名规则**：`lework-{org_public_id}-{project_public_id}`。例如 `lework-org123-prj456`。

**仓库所有者**：使用 lework 系统 gitea 账号（server config 配 token），保证所有 project 仓库由统一身份管理。gitea 端不允许 org 成员直接访问仓库，全部读写经系统账号代为执行。
...
| 项目记忆 | `.lework/memory/{agent_id}/{session_id}.json` | 纯 gitea 写入，均为 Markdown 文件 |
...
- **gitea 系统账号**：仅 lework server 持有 long-term token，配置在 server config。前端/Worker 不直接持有。
...
- **gitea 仓库可见性**：默认 private，gitea 端不允许 org 成员直接访问。lework 通过系统账号代为读写。
- **下载审计**：记录 `user_id`、`file_path`、`uri`、`ip`、`ts`。

## 8. 对现有业务流程的影响

**项目创建**：创建项目时需同步在 gitea 创建对应仓库，含 `.lework/memory/`、`assets/{artifacts,user_upload}/` 初始目录结构和 README。gitea 仓库创建失败时，项目创建事务回滚。Project 需新增 `gitea_repo_full_name` 等字段记录仓库信息。
...
**项目记忆**：项目记忆文件 `.lework/memory/` 从本地文件系统迁移至 gitea 仓库，随 git 版本化。Agent 写入后通过 git push 同步。读取可从 gitea raw API 或本地工作区获取。
...
Lework 通过 `storage-go` 库（v0.0.5）管理对象存储，`filestore` 模块（`backend/internal/infra/filestore/init.go`）是其业务封装层。`storage-go` 提供以下 driver：
...
| 配置 | gitea `[lfs]` 配置块指定 minIO 的 endpoint/bucket/access_key/secret_key；Lework `StorageConfig` 中 `driver` 从 `local` 切换为 `minio`，填写对应 endpoint/access_key/secret_key/bucket |
| 代码改造 | **零**。`driver: minio` 已在 `init.go` 注册，无需新增任何代码 |
| Worker 交互 | Worker `git clone` 通过 LFS 协议自动拉取大文件指针，Lework Server 无需感知 LFS 细节 |
...
### 10.3 方案 B：自定义 lework-lfs-pointer + S3 存储

即本文档第 3 节描述的当前方案，采用自定义 `.lfs-pointer.json` 元数据占位 + 大小阈值路由。大文件 S3 上传和 git 仓库写入均由 Worker 端完成。

### 10.4 选型对比总表

| | A1: gitea LFS + local | A2: gitea LFS + minIO | B: lework-lfs-pointer + minIO |
|---|---|---|---|
...
- **方案 B（lework-lfs-pointer）保留为备选**：当需要不依赖 gitea LFS 协议独立演进、或需要自定义阈值路由策略时可用
