# Lework Presigned URL 使用文档

## 概述

Presigned URL（预签名 URL）允许客户端直接与存储层交互，绕过后端服务完成文件上传和下载，减少服务端带宽压力。当前实现基于 `storage-go` 库，支持 `local` 和 `minio` 两种驱动。

local 驱动下 presigned URL 由 Lework 服务端生成和消费（签名 + 验证均通过 Lework）。minio 驱动下直接使用 S3 原生 presigned URL，由 MinIO 服务端处理。
...
1. 客户端向 Lework 请求预签名 URL（接口 `/v1/static/:bucket/*key?presign=1`）
2. Lework 返回带签名的 URL（`/:bucket/*key?token=...&expires=...`），有效期 1 小时
3. 客户端使用该 URL 向 Lework 发起上传/下载，Lework 验证 token 后操作底层存储

> **鉴权说明：** 生成端接口（`/v1/static/:bucket/*key`）需要鉴权，支持两种方式：
> - **JWT 登录态**：通过 `Authorization: Bearer <jwt>` 传递
> - **静态 API Key**：通过 `X-Static-Api-Key` header 传递，默认值 `lework-static-api-key`，可通过配置 `storage.static_api_key` 修改
>
...
storage:
  ...
  local_dir: lework-storage     # 本地存储目录（local 驱动）
  sign_secret: lework-local-presign  # 预签名 HMAC-SHA256 密钥
  static_api_key: "lework-static-api-key"  # 预签名生成端 API 鉴权密钥（可选，默认 lework-static-api-key）
```
...
| 变量名                   | 说明         | 默认值                           |
| ------------------------ | ------------ | -------------------------------- |
| `LEWORK_STORAGE_LOCAL_DIR` | 本地存储目录 | 可执行文件同级 `lework-storage/`   |

不配置 `storage` 字段且未设置环境变量时，自动使用 local 驱动，目录为 `$PWD/lework-storage`。
...
| `X-Static-Api-Key`     | 否   | 静态 API Key，默认 `lework-static-api-key` |
...
| 数据流经 Lework | 是（服务端验证 token 后代理读写 storage）           | 是（服务端中转）                     |
...
1. **HTTP 路由仅在 local 驱动时注册**（`router.go:117`）。minio/S3 驱动下使用原生 S3 presigned URL，Lework 不做中转。
