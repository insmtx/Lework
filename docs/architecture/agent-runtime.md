...
│   │   ├── command/          # WorkerCommand dispatcher 与 cmd.* lane handlers
│   │   │   ├── run/          # cmd.run / cmd.control：agent.run、run.cancel
│   │   │   ├── interaction/  # cmd.interaction：approval resolve、question answer
│   │   │   └── skill/        # cmd.skill：skill 管理命令，request/reply
...
Worker 命令入口。`Dispatcher` 并发启动四个 NATS command lane 订阅，subject 格式为 `org.<org_id>.worker.<worker_id>.cmd.<lane>`：

| Lane | CommandType | Handler | 确认策略 | 说明 |
| ---- | ----------- | ------- | -------- | ---- |
| `cmd.run` | `agent.run` | `command/run.Handler.HandleRunCommand` | `SubscribeManualDurable` 手动 Ack | 会话/task 执行命令。handler 先持久化本地 inbox，再 Ack；永久错误 Term，临时错误 NakWithDelay。 |
| `cmd.control` | `run.cancel` | `command/run.Handler.HandleControlCommand` | `Subscribe` 自动 Ack | 控制命令，不经过 session-keyed debounce。 |
| `cmd.interaction` | `approval.resolve`、`question.answer` | `command/interaction.Handler` | `Subscribe` 自动 Ack | 交互回传命令。 |
| `cmd.skill` | `skill.manage` | `command/skill.Handler` | `Subscribe` 自动 Ack | Skill install/list/detail/import/uninstall 等管理命令，支持 request/reply。 |

lane 选择由 `messaging.CommandLane` 定义；subject 与 consumer 名称由 `pkg/messaging` 统一生成，避免 Server 与 Worker 各自拼接协议字符串。

#### cmd.run 当前处理策略

`cmd.run` 是唯一需要手动确认的 command lane。它的目标不是同步执行完整 Agent Run，而是在消息安全落到 Worker 本地 durable inbox 后快速 Ack，再由后台 goroutine 提交给 `worker/run.Coordinator`。

这样做的目的：

- 提供至少一次（at-least-once）的崩溃恢复语义：NATS 消息只有在本地 inbox 持久化后才 Ack，Worker 重启时可以从 inbox 恢复未完成记录。
- 避免长时间 Agent Run 占住 NATS ack 路径：NATS Ack 表示“命令已被 Worker 安全接收”，不表示“Run 已执行完成”。
- 用 `topic + stream_seq` 做幂等键处理重复投递，避免同一条 JetStream 消息被重复准入。
- 把坏消息与临时失败分开：payload/config 这类不可恢复错误 `Term`，本地写入失败或关闭中的临时状态 `NakWithDelay`。
- 用 admission semaphore 和 Coordinator concurrency 控制 Worker 本地压力，用 session-keyed debounce 合并同一 session 的短时间连续输入。

当前数值与限制：

| 项 | 当前值 | 说明 |
| -- | ------ | ---- |
| `MaxConcurrency` 默认值 | `20` | `cmd/leros worker` 当前没有覆盖该值，因此使用 `command/run` 默认值；同时传给 Coordinator 作为执行并发上限。 |
| admission semaphore 容量 | `MaxConcurrency * 2`，当前默认 `40` | 限制已准入但尚未完成的异步提交数量，包含等待 Coordinator 的任务。 |
| Coordinator 执行并发 | `20` | 默认与 `MaxConcurrency` 相同；真正同时执行 Agent Run 的槽位数。 |
| session-keyed debounce window | `1500ms` | 同一 session key 的 `agent.run` 提交会走 trailing debounce 合并。 |
| `InProgress` 间隔 | `15s` | 等待 admission semaphore 时周期性延长 NATS ack wait。 |
| `NakWithDelay` 延迟 | `5s` | 缺少 stream sequence、inbox 写入失败、admission 关闭或 admission 获取失败时使用。 |
| inbox 终态清理窗口 | `72h` | `RecoverNonTerminal` 启动时清理超过 72 小时的 `completed/failed` 记录。 |
| inbox 存储 | SQLite，WAL，`busy_timeout=5000ms`，`MaxOpenConns=1` | inbox 与 provider-session bindings 当前共享 `leros.StateDBPath()`。 |
| inbox 幂等键 | `topic + stream_seq` | `stream_seq` 来自 JetStream metadata；没有该值就不能安全去重。 |
| Worker command stream 保留 | `72h`，每 subject 最多 `10000` 条，`DiscardOld` | `WORKER_CMD_STREAM` 覆盖 `cmd.run/control/interaction/skill`。 |

实时投递处理流程：

1. Dispatcher 解析 `WorkerCommand` envelope；解析失败直接 `Term`，避免坏消息反复投递。
2. `command/run.Handler` 解码 `RunCommandPayload`，校验 route、task type 和 model 配置。payload、route、task type、model/provider/API Key 等永久性错误会 `Term`。
3. 读取 JetStream metadata 的 stream sequence。没有 stream sequence 时无法做 inbox 幂等键，返回 `NakWithDelay(5s)`。
4. 获取 admission semaphore。等待期间每 15 秒调用 `InProgress`，防止 NATS ack wait 超时导致重复投递；admission 关闭或等待失败时 `NakWithDelay(5s)`。
5. 将完整 `WorkerCommand` 以 `topic + stream_seq` 为唯一键写入本地 SQLite inbox。写入失败 `NakWithDelay(5s)`。
6. 在状态锁内检查 admission 是否仍开放，注册当前进程的 `inflight` ownership，并增加后台提交的 WaitGroup 计数。
7. best-effort 标记 inbox 为 `processing`，启动后台 goroutine 调用 `Coordinator.Submit`，随后 `Ack` NATS 消息。
8. 后台执行完成后，成功标记 inbox 为 `completed`，失败标记为 `failed`，并释放 admission 与 inflight。

重复投递按 inbox 状态处理：

- 已有终态记录（`completed` 或 `failed`）：直接 `Ack`。
- 已有非终态记录且当前进程已拥有 inflight：直接 `Ack`。
- 已有非终态但没有当前进程 ownership：视为 stale record，由当前进程重新接管并异步提交。

崩溃恢复在启动 NATS 订阅前执行：`RecoverNonTerminal` 先清理超过 72 小时的终态记录，再按 `stream_seq` 顺序加载非终态 inbox 记录，注册为当前进程 ownership，并通过与实时投递相同的 admission semaphore 逐条恢复提交。

`cmd.run` handler 只负责消息准入、持久化、恢复和异步提交；真正的 session-keyed debounce、执行并发限制、active run tracking 与 cancel 由 `worker/run.Coordinator` 负责。
